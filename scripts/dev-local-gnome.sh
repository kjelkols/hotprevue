...
# Bruk: bash scripts/dev-local-notmux.sh
...
gnome-terminal --title="Backend"  -- bash -c "cd $REPO/backend  && DATABASE_URL=$DATABASE_URL COLDPREVIEW_DIR=$COLDPREVIEW_DIR AI_SEARCH_URL=http://tenketank.tail764ab5.ts.net:8001 uv run uvicorn main:app --reload --host 0.0.0.0 --port 8000; exec bash"
