...
| `sharpness_score` | float (nullable) | Laplacian-varians — høy = skarpt. Beregnet fra original ved registrering. |
| `exposure_mean` | float (nullable) | Gjennomsnittlig lysstyrke 0–255 (128 = nøytral). |
| `exposure_clipping` | float (nullable) | Andel klippede piksler (< 5 og > 250). |
| `noise_score` | float (nullable) | Støyestimat fra 512×512-patch fra original. |
| `registered_at` | datetime | — |
| `deleted_at` | datetime (nullable) | Null = aktiv. Satt = mykt slettet. Hard-slettes via `empty-trash`. |

Se ADR-021 for detaljer om kvalitetsberegning. Alle kvalitetsfelter er nullable — eksisterende bilder registrert før ADR-021 mangler disse verdiene.
...
Én rad per Photo som har visningskorreksjoner. Tabellen er sparse — kun Photos med aktive korreksjoner har en rad. Korreksjoner appliseres på-farten i `serve_coldpreview()` uten å modifisere originalfiler. Se ADR-028.

Pipeline-rekkefølge: rotation → flip_horizontal → horizon_angle → crop → exposure_ev.

| Felt | Type | Beskrivelse |
|---|---|---|
| `photo_id` | UUID FK (unique) | Tilhørende Photo — én rad per Photo |
| `rotation` | int (nullable) | `90`, `180`, `270` grader — helrotasjon |
| `flip_horizontal` | bool | Speilvend horisontalt — for skannet negativ/lysbilder lagt inn baklengs |
| `horizon_angle` | float (nullable) | Grader å rette opp horisont (±15°, bicubic, auto-crop av svarte hjørner) |
| `exposure_ev` | float (nullable) | EV-justering (log2-skala), f.eks. `+0.5`, `-1.0` |
...
| `updated_at` | datetime | Oppdateres ved enhver endring — brukes i ETag for cache-invalidering |
