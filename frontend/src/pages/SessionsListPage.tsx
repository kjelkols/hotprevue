    ...
    <div className="min-h-full bg-gray-950 text-white">
      ...
      <div className="p-4 max-w-2xl mx-auto">
        ...
        {!isLoading && sessions.length === 0 && (
          <p className="text-gray-400 py-8 text-center">Ingen sesjoner registrert ennå.</p>
        )}
            ...
            <li key={session.id}>
              ...
              >
                ...
                <div className="flex items-baseline gap-3 mt-0.5">
                  <span className="text-sm text-gray-400 truncate">{session.source_path}</span>
                  <span className="text-xs text-gray-500 shrink-0">{formatDate(session.started_at)}</span>
                </div>
              </button>
            </li>
          ))}
